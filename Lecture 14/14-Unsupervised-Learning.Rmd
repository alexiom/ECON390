...
subtitle: "Lecture 14: Unsupervised Learning"
author: "Alex Marsh"
...
```{r,echo=F}
ggplot(NHIS_data,aes(x=HEIGHTTC_A))+geom_density()
```
...
```{r,echo=F}
pdf = function(x){
  dnorm(x,theta1[1],theta1[3])*theta1[5]+
    dnorm(x,theta1[2],theta1[4])*(1-theta1[5])
}

xs  = seq(min(heights)-5,max(heights)+5,0.5)
xsM = seq(min(NHIS_data[SEX_A==1,HEIGHTTC_A])-5,
          max(NHIS_data[SEX_A==1,HEIGHTTC_A])+5,0.5)
xsF = seq(min(NHIS_data[SEX_A==2,HEIGHTTC_A])-5,
          max(NHIS_data[SEX_A==2,HEIGHTTC_A])+5,0.5)

ys  = pdf(xs)
ysM = dnorm(xsM,theta1[1],theta1[3])
ysF = dnorm(xsF,theta1[2],theta1[4])

plot_group = c(rep("Mixture",length(xs)),
               rep("Male",length(xsM)),
               rep("Female",length(xsF)))

plot_data = data.table(y=c(ys,ysM,ysF),
                       x=c(xs,xsM,xsF),
                       group=plot_group)

final_plot = ggplot(plot_data,aes(x=x,y=y,color=plot_group))+geom_line()
```

```{r,echo=F}
final_plot + ggtitle("Final Mixture Distributions") + 
  theme_bw() + theme(plot.title = element_text(hjust=0.5)) +
  xlab("Height") + ylab("Density") +
  guides(color=guide_legend(title="Distribution")) + 
  scale_x_continuous(breaks=seq(round(min(xs),-1),round(max(xs),-1),5))
```
